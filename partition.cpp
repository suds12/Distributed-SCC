...
int main(int argc, char *argv[]){
    ...
    if (argc < 3) {
        cerr << "Usage: ./partition inputgraph num_part\n\tinputgraph: edgelist graph input\n\tnum_part: number of partitions, e.g., 4 or a comma-separated list of values, e.g., 2,4,8,16\n";
        exit(1);
    }
    string filename = argv[1];
    string partstr(argv[2]);
    vector<int> nparts;
    stringstream ss(partstr);
    string token;
    while (std::getline(ss, token, ','))  
       nparts.push_back(stoi(token)); 
    ...
    for (auto it = nparts.begin(); it != nparts.end(); it++) {
        int nParts = *it;
